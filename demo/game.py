...
class NarutoGame(object):
    def __init__(self, voice=True):
        ...
        self.model = load_model("/Users/anitahu/htn19/htn19-backend/data/custom_sign_model_v3.h5")
        self.class_names = ["bird", "boar", "dog", "dragon", "hare", "horse", "monkey", "ox", "ram", "rat", "serpant", "tiger", "none"]
        self.jutsu_names = ["Start Game", "Lightning Dragon Jutsu", "Fire Ball Jutsu", "Rasengan", "Rasenshuriken", "Water Shark", "None"]
        self.jutsus = {"2": 3, "8": 5, "5": 2, "10": 1, "6": 4}
    ...
    def detect_cards(frame, curr_player):
        ...
        for cont in contours:
            if cv2.contourArea(cont) > largest:
                largest = cv2.contourArea(cont)
                max_bounding = cv2.boundingRect(cont)

        cv2.rectangle(frame, (int(max_bounding[0]), int(max_bounding[1])), \
              (int(max_bounding[0]+max_bounding[2]), int(max_bounding[1]+max_bounding[3])), (0,0,255), 2)

        if largest > 10000:
            frame = draw_label(frame, "Special card detected!",(0, int(frame.shape[0]*0.95)), 1, (255,255,0))
    ...
    def get_frame(self, meta):
        ...
        if ret:
            ...
            if not start_game:
                ...
                # Draw player stats
                frame = draw_label(frame, f"Round {self.game_round}", (0, 30), 1, (255,255,0))
                frame = draw_label(frame, "Naruto" if self.curr_player == 1 else "Sasuke", (int(width*0.8), int(height*0.55)), 1.5, (255,255,0))
                # frame = draw_label(frame, f"HP: {self.player_stats[0]['hp']}",(int(width*0.8), int(height*0.6)), 0.8, (255,255,0))
                # frame = draw_label(frame, f"Chakra: {self.player_stats[0]['chakra']}",(int(width*0.8), int(height*0.65)), 0.8, (255,255,0))
                # frame = draw_label(frame, f"Specials: {self.player_stats[0]['specials']}",(int(width*0.8), int(height*0.7)), 0.8, (255,255,0))
                #
                # frame = draw_label(frame, "Sasuke", (int(width*0.8), int(height*0.8)), 1, (255,255,0))
                # frame = draw_label(frame, f"HP: {self.player_stats[1]['hp']}",(int(width*0.8), int(height*0.85)), 0.8, (255,255,0))
                # frame = draw_label(frame, f"Chakra: {self.player_stats[1]['chakra']}",(int(width*0.8), int(height*0.9)), 0.8, (255,255,0))
                # frame = draw_label(frame, f"Specials: {self.player_stats[1]['specials']}",(int(width*0.8), int(height*0.95)), 0.8, (255,255,0))
                ...
                if self.detect_cards(frame, self.curr_player) and self.player_stats[self.curr_player-1]['specials'] == 100:
                    ...
                    if self.player_switch:
                        if self.player_stats[0]['hp'] == 0:
                            self.player_stats[0]['hp'] == 100
                        if self.player_stats[1]['hp'] == 0:
                            self.player_stats[1]['hp'] == 100

                        attack_name = self.jutsu_names[self.prev_move]
                        frame = draw_label(frame, f"Performed Attack: {attack_name}",(0, int(frame.shape[0]*0.85)), 1.3, (255,255,0))
                        frame = draw_label(frame, "Player switch!!",(0, int(height*0.9)), 1.3, (255,255,0))
                        ...
                        # 20s count down
                        time_elapse = int(time.time() - self.time_start)
                        cv2.putText(frame, f"{25 - time_elapse}",(width//2, 60), font, 2,(0,0,255), 2)
                        ...
                        if self.curr_combo != []:
                            ...
                            selected = -1
                            for start in self.jutsus.keys():
                                if start in jutsu:
                                    selected = start

                            if selected != -1 and passed:
                                print(f"{self.jutsus[selected]}!!")
                                cv2.putText(frame, f"{self.jutsu_names[self.jutsus[selected]]}!!",(frame.shape[1]//3, int(frame.shape[0]*0.9)), font, 1.5,(255,255,0), 2)
                                self.prev_move = self.jutsus[selected]
                                self.player_stats[self.curr_player-1]['attack'] = self.jutsus[selected]
                                # Update stats
